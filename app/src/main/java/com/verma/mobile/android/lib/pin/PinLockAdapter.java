...
public class PinLockAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == VIEW_TYPE_NUMBER) {
            ...
            viewHolder = new PinLockAdapter.NumberViewHolder(view);
        } else {
            View view = inflater.inflate(R.layout.layout_delete_item, parent, false);
            viewHolder = new PinLockAdapter.DeleteViewHolder(view);
        }
        return viewHolder;
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder.getItemViewType() == VIEW_TYPE_NUMBER) {
            PinLockAdapter.NumberViewHolder vh1 = (PinLockAdapter.NumberViewHolder) holder;
            configureNumberButtonHolder(vh1, position);
        } else if (holder.getItemViewType() == VIEW_TYPE_DELETE) {
            PinLockAdapter.DeleteViewHolder vh2 = (PinLockAdapter.DeleteViewHolder) holder;
            configureDeleteButtonHolder(vh2);
        }
    }

    private void configureNumberButtonHolder(PinLockAdapter.NumberViewHolder holder, int position) {
        if (holder != null) {
            if (position == 9) {
    ...
    private void configureDeleteButtonHolder(PinLockAdapter.DeleteViewHolder holder) {
        if (holder != null) {
    ...
    public PinLockAdapter.OnNumberClickListener getOnItemClickListener() {
        return mOnNumberClickListener;
    }

    public void setOnItemClickListener(PinLockAdapter.OnNumberClickListener onNumberClickListener) {
        this.mOnNumberClickListener = onNumberClickListener;
    }

    public PinLockAdapter.OnDeleteClickListener getOnDeleteClickListener() {
            ...
            mNumberButton.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if (mOnNumberClickListener != null) {
                        mOnNumberClickListener.onNumberClicked((Integer) v.getTag(),itemView);
                    }
                }
            });
        }
    ...
    public interface OnNumberClickListener {
        void onNumberClicked(int keyValue, View view);
    }
